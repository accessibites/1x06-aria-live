...
function accessibleAlert(text) {
		var divStatus = $("#playerStatusDiv");
		if (divStatus.length == 0)
		{
			divStatus = $('<div id="playerStatusDiv" role="status"></div>');
			$("body").append(divStatus);
			
		}
		divStatus.html(text);
		setTimeout(function () { divStatus.empty(); }, 100);
}
...
function addAccessibleControls(audioWrapper) {
	...
	var ddPosition = $('<span class="ddPosition" aria-live="off">' + _('0 segundos') + '</span>');
